...
from vid_configuration import SHOW_DETECT, DATA_RECORD, RE_CHECK, RE_START_TIME, RE_END_TIME, SD_CHECK, SHOW_VIOLATION_COUNT, SHOW_TRACKING_ID, SOCIAL_DISTANCE,\
...
def _record_crowd_data(time, human_count, violate_count, restricted_entry, abnormal_activity, crowd_data_writer):
    data = [time, human_count, violate_count, int(
        restricted_entry), int(abnormal_activity)]
...
# MAin function to process video and create a excel file
...
def video_process(cap, frame_size, net, ln, encoder, tracker, movement_data_writer, crowd_data_writer):
    ...
    while True:
        ...
        [humans_detected, expired] = detect_human(
            net, ln, frame, encoder, tracker, record_time)
        ...
        if SHOW_PROCESSING_OUTPUT or SHOW_DETECT or SD_CHECK or RE_CHECK or ABNORMAL_CHECK:
            ...
            for i, track in enumerate(humans_detected):
                ...
                if SD_CHECK:
                    if len(humans_detected) >= 2:
                        ...
                        for j, track_2 in enumerate(humans_detected[i+1:], start=i+1):
                            if HIGH_CAM:
                                [cx_2, cy_2] = list(
                                    map(int, track_2.positions[-1]))
                                distance = euclidean((cx, cy), (cx_2, cy_2))
                            else:
                                [x_2, y_2, w_2, h_2] = list(
                                    map(int, track_2.to_tlbr().tolist()))
                ...
                if ABNORMAL_CHECK:
                    ke = kinetic_energy(
                        track.positions[-1], track.positions[-2], TIME_STEP)
                ...
                if i in violate_set:
                    ...
                    if SHOW_VIOLATION_COUNT:
                        cv2.putText(frame, str(int(
                            violate_count[i])), (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, RGB_COLORS["green"], 2)
                ...
                if SHOW_TRACKING_ID:
                    cv2.putText(frame, str(int(idx)), (x, y - 10),
        ...
        if DATA_RECORD:
            _record_crowd_data(record_time, len(humans_detected), len(
                violate_set), RE, ABNORMAL, crowd_data_writer)
